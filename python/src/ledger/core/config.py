...
class LedgerConfig(BaseSettings):
    ...
    flush_interval: float = Field(
        default=5.0,
        gt=0,
        description="Interval in seconds between automatic buffer flushes"
    )

    flush_size: int = Field(
        default=100,
        gt=0,
        description="Number of logs that trigger an automatic flush"
    )

    max_buffer_size: int = Field(
        default=10000,
        gt=0,
        description="Maximum number of logs to keep in buffer"
    )
    ...
    rate_limit_buffer: float = Field(
        ...
        gt=0,
        le=1,
        description="Rate limit buffer percentage"
    )
