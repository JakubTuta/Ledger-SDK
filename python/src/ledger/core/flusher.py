...
class BackgroundFlusher:
    ...
    async def _run(self) -> None:
        while not self._shutdown_event.is_set():
            try:
                ...
                while not self.buffer.spans_empty() and not self._shutdown_event.is_set():
    ...
    async def _flush_spans_once(self) -> bool:
        ...
        try:
            await self.rate_limiter.wait_if_needed()
            response = await self.http_client.post(
                "/api/v1/ingest/spans/batch",
                json_data={"spans": batch},
            )
            if response.status_code == 202:
                return True
            if response.status_code == 400:
                logging_module.get_logger().error(
                    "Spans batch rejected (400): %s", response.text[:500]
                )
                return True
            self.buffer.requeue_spans(batch)
            return False
        except Exception as e:
            logging_module.get_logger().error("Error flushing spans: %s", e)
            self.buffer.requeue_spans(batch)
            return False
    ...
    async def shutdown(self, timeout: float = 10.0) -> None:
        ...
        while not self.buffer.spans_empty():
            remaining = deadline - time.monotonic()
            if remaining <= 0:
                break
            batch = self.buffer.get_span_batch(self.max_batch_size)
